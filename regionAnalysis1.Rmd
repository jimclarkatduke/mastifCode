...
nc      <- max( sapply(dfScaled, ncol) )
#width <- rep(0, length(dfScaled))

width <- matrix( 0, length(dfScaled), nc)
rownames(width) <- names(dfScaled)
keep  <- numeric(0)
...
#width <- width + .1
width[ width > 10 ] <- 10  # intensity scaled by posterior
width <- 1 - width/max(width)
width[ width < .1] <- .1
width[ width > .9] <- .9
...
par(mfrow=c(2,1), bty='n', mar=c(2,2,3,1), omi=c(1,.8,0,.5))
plot( NA, xlim=c(0,1.5), ylim = c(0, 1), xlab='',ylab = '' )

for(k in io){
  df <- dfScaled[[io[k]]][1:2,]
  if(length(df) == 0)next
  nt <- ncol(df)
  wk <- width[io[k], 1:nt]^3
  wk[wk < .05] <- .05
  
  segments( df[1,1:(nt-1)], df[2,1:(nt-1)], df[1,2:nt], df[2,2:nt],
          col = .getColor('black', wk[1:(nt-1)]), lwd = 2 )
  
 # lines(df[1,], df[2,], lwd = 1, 
...
for(k in ii){
  df <- dfScaled[[ii[k]]][1:2,]
  if(length(df) == 0)next
  nt <- ncol(df)
  wk <- width[ii[k], 1:nt]^3
  wk[wk < .05] <- .05
