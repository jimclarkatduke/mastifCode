...
modelFile <- 'modelTemperate.csv'   # file of predictors
...
if(region == 'west'){
  ...
  skipGen <- c("ailanthus","alnus","amelanchier",
               "celtis","chamaecyparis", "cupressus","ficus",
...
for(t in ie:length(genFull)){
  ...
  if(is.null(treeData))next     # no seeds or cropCounts
  ...
  # predictors
  ttt <- getPredictors( modelFile = "modelTemperate.csv", 
...
ft <- paste(path, opath, regions[1], sep='')
files <- list.files(ft, full.names=T)
gen   <- list.files(ft)

genFull <- columnSplit(gen,'.Rdata')[,1]

ft <- paste(path, opath, regions[2], sep='')
files <- list.files(ft, full.names=T)
gen   <- list.files(ft)

genFull <- c(genFull, columnSplit(gen,'.Rdata')[,1])
#genCode <- substr(genFull, 1, 4)
...
for(t in 1:length(genFull)){
  ...
  for(r in 1:2){
    ...
    fchain <- output$chains$bfec  # unstandardized 
    wbd    <- which( endsWith( colnames(fchain), ':diam' ) )
    wb2    <- which( endsWith( colnames(fchain), ':I(diam^2)' ) )
    bd     <- fchain[,wbd, drop=F]
    b2     <- fchain[,wb2, drop=F]
    dopt   <- -bd/2/b2
    
    cnames <- .replaceString( colnames(dopt), 'species','')
    cnames <- .replaceString( cnames, ':diam','')
    
    colnames(dopt) <- cnames
    
    rchain <- output$chains$brep  # unstandardized 
    wbd    <- which( endsWith( colnames(rchain), ':diam' ) )
    wbi    <- c(1:ncol(rchain))[ -wbd ]
    rd     <- rchain[,wbd, drop=F]
    ri     <- rchain[,wbi, drop=F]
    
    rhalf <- -ri/rd
    colnames(rhalf) <- cnames
    
    dq <- t( round( apply( dopt, 2, quantile, c(.5, .025, .975) ), 2) )
    rq <- t( round( apply( rhalf, 2, quantile, c(.5, .025, .975) ), 2) )
    
    colnames(dq) <- paste( 'dopt',colnames(dq) )
    colnames(rq) <- paste( 'rhalf',colnames(rq) )
    
    dr <- cbind(dq, rq)
    ss <- numeric(0)
    
    for(j in 1:ncol(rhalf)){
      
      scov <- matrix( signif( cov( cbind(rhalf[,j],dopt[,j]) )[ c(1,4,3) ], 4 ), 1 )
      colnames(scov) <- c('varDopt','varRep','covDR')
      ss <- rbind(ss, scov)
    }
    dr <- cbind(dr, ss)
    
    rownames(dr) <- paste( rownames(dr), reg, sep='_' )
    
    data <- rbind(data, dr)
    
 #   maxDiam <- tapply( output$inputs$treeData$diam, 
 #                      output$inputs$treeData$species,
 #                      max, na.rm=T )
  }
}
